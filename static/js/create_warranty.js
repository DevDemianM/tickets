...
function setupPartsTable() {
    ...
    function selectPartAndUpdateRow(partData) {
        ...
        try {
            // Obtener los elementos de la fila - ahora son inputs en lugar de selects
            ...
            if (!hiddenInput) {
                throw new Error("No se encontró el campo oculto de código");
            }
            ...
            if (modal) {
                ...
                const newModal = new bootstrap.Modal(searchPartsModal);
                newModal.hide();
            }
            
            // Limpiar backdrop de manera segura
            searchPartsModal.addEventListener('hidden.bs.modal', function() {
...
// Configurar confirmación para crear garantía
function setupCreateWarrantyConfirmation() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            // Primero ejecutar validaciones si existen
            if (typeof validateForm === 'function' && !validateForm()) {
                return false;
            }
            
            Swal.fire({
                title: '¿Crear ticket de garantía?',
...
// Cambia la validación del IMEI para garantías
function validateIMEI(imei) {
    ...
    // Permitir cualquier cantidad de caracteres, letras o números
    if (!/^[A-Za-z0-9]+$/.test(imei.trim())) {
        return {
            isValid: false,
            message: 'El IMEI/Serial solo puede contener letras y números'
        };
    }
    return {
        isValid: true,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Configurar confirmación para crear garantía
    setupCreateWarrantyConfirmation();
